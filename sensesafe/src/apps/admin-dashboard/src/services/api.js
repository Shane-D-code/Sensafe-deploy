...
export const getAllSOSAlerts = async () => {
  try {
    // Use admin SOS endpoint to get ALL SOS including anonymous
    const response = await apiClient.get('/api/admin/sos', {
      params: { page_size: 100 }
    });
    return response.data.sos_alerts || [];
  } catch (error) {
    console.warn('Could not fetch SOS from admin endpoint:', error.message);
    // Fallback: try user endpoint (will only return authenticated user's SOS)
    try {
      const response = await apiClient.get('/api/sos/user');
      return response.data.sos_alerts || [];
    } catch (e) {
...
export const getAllIncidentsDirect = async () => {
  try {
    // Use admin incidents endpoint to get ALL incidents including anonymous
    const response = await apiClient.get('/api/admin/incidents', {
      params: { page_size: 100 }
    });
    return response.data.incidents || [];
  } catch (error) {
    console.warn('Could not fetch incidents from admin endpoint:', error.message);
    // Fallback: try user endpoint (will only return authenticated user's incidents)
    try {
      const response = await apiClient.get('/api/incidents/user');
      return response.data.incidents || [];
    } catch (e) {
...
export const getAllAlertsForAdmin = async () => {
  try {
    console.log('🔄 Fetching all alerts from backend (Admin Mode)...');

    // Fetch all sources in parallel - using ADMIN endpoints
    const [messagesRes, sosRes, incidentsRes] = await Promise.allSettled([
      apiClient.get('/api/messages/admin/all', { params: { page_size: 100 } }),
      apiClient.get('/api/admin/sos', { params: { page_size: 100 } }),  // ADMIN endpoint
      apiClient.get('/api/admin/incidents', { params: { page_size: 100 } })  // ADMIN endpoint
    ]);
 ...
 * @param {number} days - Number of days to include (default: 7, max: 30)
 * @returns {Promise<Object>} Time-series data for alerts
 */
export const getAlertsOverTime = async (days = 7) => {
  try {
    const response = await apiClient.get(`/api/admin/analytics/alerts-over-time?days=${days}`);
    return response.data;
  } catch (error) {
    console.error('Get alerts over time error:', error);
    throw error;
  }
};
